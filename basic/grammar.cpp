...
bool ConcreteRule::equal(const Rule& other) const {
...
std::vector<PProgram> grammar::generateHeightLimitedProgram(Grammar* grammar_original, int limit) {
    ...
    // sub_node -> node & rule from node to sub_node
    std::vector<std::vector<std::pair<NonTerminal*, Rule*>>> reversed_edge(n);
    for (auto* symbol: grammar->symbol_list) {
        for (auto* rule: symbol->rule_list) {
            std::set<int> sub_set;
            for (auto* sub: rule->param_list) {
                if (sub_set.find(sub->id) == sub_set.end()) {
                    reversed_edge[sub->id].emplace_back(symbol, rule);
                    sub_set.insert(sub->id);
                }
            }
        }
    }
    ...
    for (auto* symbol: grammar->symbol_list) {
        // whether this symbol has empty rule, if have then depth[symbol] = 1 and add program into res[symbol->id]
        bool has_empty_rule = false;
        for (auto* rule: symbol->rule_list) {
            if (rule->param_list.empty()) {
                ProgramList empty_list;
                res[symbol->id].push_back(rule->buildProgram(empty_list));
                has_empty_rule = true;
            }
        }
        if (has_empty_rule) {
            ...
            Q_set.insert(symbol->id);
        }
    }

    while (!Q.empty()) {
        auto k = Q.top(); Q.pop(); Q_set.erase(k.first);
        if (d[k.second] != k.first) continue;
        for (auto& [node, edge]: reversed_edge[k.second]) {
            int size = d[node->id];
            for (auto* sub: edge->param_list) {
                size = std::min(size, 1 + d[sub->id]);
            }
            if (size <= d[node->id]) {
                // update node depth
                d[node->id] = size;
                ...
                for (auto& sub_list: sub_lists) {
                    res[node->id].push_back(edge->buildProgram(sub_list));
                }
                // if node->id not in Q, then add into Q; else don't add
                if (Q_set.find(node->id) == Q_set.end()) {
                    Q.push({d[node->id], node->id});
                    Q_set.insert(node->id);
                }
            }
        }
    }
...
void grammar::deleteDuplicateRule(std::vector<Rule*>& rule_list) {
...
void grammar::deleteDuplicateRule(Grammar* grammar) {
    for (auto& symbol: grammar->symbol_list) {
