...
InputComponentInfo::InputComponentInfo(const std::string &_name, bool _is_compress_related, bool _is_recursive, int _command_id):
    name(_name), is_compress_related(_is_compress_related), is_recursive(_is_recursive), command_id(_command_id) {
}
InputComponentInfo::InputComponentInfo(const std::string &_name): name(_name),
    is_compress_related(false), is_recursive(false), command_id(-1) {
}
...
std::unordered_map<std::string, InputComponentInfo> incre::getComponentInfo(const IncreProgram &program) {
    ...
    recursive_task.term_matcher = [](TermData* term, const MatchContext& ctx) -> bool {
        return term->getType() == TermType::FIX;
    };
    auto compress_info = match::match(program.get(), compress_task), recursive_info = match::match(program.get(), recursive_task);
    ...
    // Merge all infos
    std::unordered_map<std::string, InputComponentInfo> res;
    ...
    for (auto& [name, command_id]: id_map) {
        ...
        res[name] = InputComponentInfo(name, compress_info[name], recursive_info[name], command_id);
    }
    return res;
}